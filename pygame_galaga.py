...
def playEnemy0(enemy0_speed, time_now):
    ...
    if random.randrange(0, 100) < enemy_persentage[0]:  # 1~100 랜덤 돌려서 나온 숫자가 적0 퍼센테이지 값보다 높다면
...
def createEnemy1():
    ...
    if enemy_x<=pad_width/2:
        ...
        enemy_z=-1
    for i in range(3):
        enemy_xy[1].append([enemy_x, enemy_y, enemy_z])  #적 xy좌표 리스트 적0 리스트 부분에 생성할 새로운 적의 xy 좌표 리스트 추가
        enemy_y += -(enemy_height+5)    #맨 앞 적 뒤에 따라 나오는 적들 y값 (바로 앞의 적 y값) - (적 세로 높이+간격)으로 설정, 더 큰 음수로 설정하는 좌표상 더 위에 생성해야해서
...
def playEnemy1(enemy1_speed, time_now):
    ...
    if time_now > nextLevel[1]:
        enemy_persentage[1]+10 #적0 등장확률 증가
        nextLevel[1]+=10    #적0 다음 레벨로 넘어가는 기준값 증가
    #적 생성
    #if len(enemy_xy[1]) == 0:  #적 생성 후 생성한 적이 사라질때까지 새로운 시퀀스를 생성하지 않을시 이 코드 추가
    if random.randrange(0, 100) < enemy_persentage[1]:  # 1~100 랜덤 돌려서 나온 숫자가 적0 퍼센테이지 값보다 높다면
        createEnemy1()  #적0 생성 함수 실행

    for i, exy in enumerate(enemy_xy[1]):   #i: 현재 접근중인 인덱스값, exy: 현재 접근중인 적 좌표 리스트 [x,y]
        #적0 y좌표 변경
        exy[1] += enemy1_speed #적0의 스피드만큼 y값 이동
...
def createEnemy2():
    ...
    if enemy_x<=pad_width/2:
        ...
        enemy_z=-1
    for i in range(3):
        enemy_xy[2].append([enemy_x, enemy_y, enemy_z])  #적 xy좌표 리스트 적0 리스트 부분에 생성할 새로운 적의 xy 좌표 리스트 추가
        enemy_y += -(enemy_height+5)    #맨 앞 적 뒤에 따라 나오는 적들 y값 (바로 앞의 적 y값) - (적 세로 높이+간격)으로 설정, 더 큰 음수로 설정하는 좌표상 더 위에 생성해야해서
...
def playEnemy2(enemy2_speed, time_now):
    ...
    if time_now > nextLevel[2]:
        enemy_persentage[2]+10 #적0 등장확률 증가
        nextLevel[2]+=10    #적0 다음 레벨로 넘어가는 기준값 증가
    #적 생성
    #if len(enemy_xy[2]) == 0:  #적 생성 후 생성한 적이 사라질때까지 새로운 시퀀스를 생성하지 않을시 이 코드 추가
    if random.randrange(0, 100) < enemy_persentage[2]:  # 1~100 랜덤 돌려서 나온 숫자가 적0 퍼센테이지 값보다 높다면
        createEnemy2()  #적0 생성 함수 실행

    for i, exy in enumerate(enemy_xy[2]):   #i: 현재 접근중인 인덱스값, exy: 현재 접근중인 적 좌표 리스트 [x,y]
        #적0 y좌표 변경
        exy[1] += enemy2_speed #적0의 스피드만큼 y값 이동
...
# 적3 생성 함수
def createEnemy3():
    enemy_x = random.choice([0, pad_width-enemy_width])    #적 위치 랜덤 x좌표로 지정
    enemy_y = random.randrange(pad_height/2, pad_height-enemy_height)  #적 위치 랜덤 y좌표로 지정
    enemy_z = 0 #적 방향 지정 (화면 좌측에선 1, 우측에선 -1)
    if enemy_x<=pad_width/2:
        enemy_z=1
    else:
        enemy_z=-1
    for i in range(3):
        enemy_xy[3].append([enemy_x, enemy_y, enemy_z])  #적 xy좌표 리스트 적0 리스트 부분에 생성할 새로운 적의 xy 좌표 리스트 추가
...
def playEnemy3(enemy3_speed, time_now):
    ...
    if time_now > nextLevel[3]:
        enemy_persentage[3]+10 #적0 등장확률 증가
        nextLevel[3]+=10    #적0 다음 레벨로 넘어가는 기준값 증가
    #적 생성
    #if len(enemy_xy[2]) == 0:  #적 생성 후 생성한 적이 사라질때까지 새로운 시퀀스를 생성하지 않을시 이 코드 추가
    if random.randrange(0, 100) < enemy_persentage[3]:  # 1~100 랜덤 돌려서 나온 숫자가 적0 퍼센테이지 값보다 높다면
        createEnemy3()  #적0 생성 함수 실행
...
def createEnemy4():
    ...
    if enemy_x<=pad_width/2:
        ...
        enemy_z=-1
    for i in range(3):
        enemy_xy[4].append([enemy_x, enemy_y, enemy_z])  #적 xy좌표 리스트 적0 리스트 부분에 생성할 새로운 적의 xy 좌표 리스트 추가
        enemy_y += -(enemy_height+5)    #맨 앞 적 뒤에 따라 나오는 적들 y값 (바로 앞의 적 y값) - (적 세로 높이+간격)으로 설정, 더 큰 음수로 설정하는 좌표상 더 위에 생성해야해서
...
def playEnemy4(enemy4_speed, time_now):
    ...
    if time_now > nextLevel[4]:
        enemy_persentage[4]+10 #적0 등장확률 증가
        nextLevel[4]+=10    #적0 다음 레벨로 넘어가는 기준값 증가
    #적 생성
    #if len(enemy_xy[4]) == 0:  #적 생성 후 생성한 적이 사라질때까지 새로운 시퀀스를 생성하지 않을시 이 코드 추가
    if random.randrange(0, 100) < enemy_persentage[4]:  # 1~100 랜덤 돌려서 나온 숫자가 적0 퍼센테이지 값보다 높다면
        createEnemy4()  #적0 생성 함수 실행

    for i, exy in enumerate(enemy_xy[4]):   #i: 현재 접근중인 인덱스값, exy: 현재 접근중인 적 좌표 리스트 [x,y]
        #적0 y좌표 변경
        #갤러리안과 떨어져 있다면 갤러리안의 x좌표로 이동
        if y-exy[1]>pad_height/4:
            exy[1] += enemy4_speed #적0의 스피드만큼 y값 이동
            if exy[0]<x:
